...
# AJUSTE PROPUESTO POR LA REGIONAL: agregar el umbral (sin considerar el tiempo) para los que usan el transporte particular o colectivo informal, transporte humanitario, cola/aventón; u Otro. 
#Para replicar el calculo con los ajustes quitar de comentarios el chunk y poner como comentario el anterior (versión original)
...
main_merged <- main_merged |>
  ...
  mutate(R_CSI = (FS_D2_Q1 * 1 + FS_D2_Q2 * 1 + 
                    ...
                    FS_D2_Q5 * 2)) |>
  #se hacen cambios en FS_D2_Q2 * 2 se cambia por 1 / FS_D2_Q5 * 1 por 2
  ...
  mutate(Mean_coping_capacity = rowMeans(across(c(Max_coping_behaviour)), na.rm = TRUE),
          CARI_unrounded_NO_FES = rowMeans(across(c(FCS_rCSI, Mean_coping_capacity)), na.rm = TRUE),
         CARI_NO_FES = round_half_up(CARI_unrounded_NO_FES))|> # do not use simple function "round"
  mutate(IND_FS_max = ifelse(CARI_NO_FES > 2, 1, 0))
#para el Pin Cari_no_FES, se deja planteada la linea 160, si activa esta, se debe desactivar la 156 para replicar el PiN de FS sin FES / cuidado con pipe de la 159
...
# Protection from Sexual Eploitation and Abuse (PSEA)
# PSEA_D2: % of households that have not received basic messages on SEA prevention
main_merged <- main_merged |>
  mutate(PSEA_D2 = ifelse(psea3 == "1", 1, 0))|>
  mutate(IND_PSEA_max = PSEA_D2)
